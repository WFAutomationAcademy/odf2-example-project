...
class EmailMonitorTaskTest {
    ...
    void setUp(OrmSupport ormSupport, S3MockClient s3MockClient) {
        ormSupport.createTables(Transaction.class, Email.class, Attachment.class,
                MonitorStateEntity.class, MonitorConfigurationEntity.class, ConfigEntity.class);

        ormSupport.getConfigRepository().create(new ConfigEntity("example.attachments.bucket.name", "test-bucket"));
    ...
    void shouldGetNewEmailsFromExternalService(BotTaskFactory botTaskFactory, OrmSupport ormSupport, S3MockClient s3MockClient) {
        // when
        List<Map<String, String>> stepRecords = botTaskFactory.fromClass(EmailMonitorTask.class).buildAndRun().getRecords();
